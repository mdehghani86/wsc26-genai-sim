...
def _log(env: simpy.Environment, log: list[str], pid: int | None, msg: str,
         verbose: bool) -> None:
    """Skip log append entirely when not verbose so long runs do not OOM the
    in-browser Python heap with millions of trace lines."""
    if not verbose:
        return
...
def queue_sampler(env: simpy.Environment,
    ...
    """Bounded-memory queue sampler.

    For short runs (<= max_samples minutes) we keep the original 1-min cadence.
    For long runs we coarsen the sampling step so the stored list never grows
    past ``max_samples`` entries. This avoids OOM in browser-side Pyodide on
    multi-million-minute runs while keeping the time-averaged mean accurate.
    """
    if duration and duration > max_samples * step:
...
def run_single_simulation(settings: dict | None = None) -> RunResult:
    ...
    env.process(queue_sampler(env, waiting_lists, critical_stats, standard_stats, duration=cfg["duration"]))
